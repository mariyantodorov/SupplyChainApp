...
contract('SupplyChain', async accounts => {
  it("should create a Participant", async () => {
    ...
    let participantId = await instance.addParticipant("A","passA","0x7E2F6Cf7a61f0A3Fd2AcA16a74f7f8Fcb8253940","Manufacturer");
    let participant = await instance.participants(0);
    assert.equal(participant[0], "A");
    assert.equal(participant[2], "Manufacturer");

    participantId = await instance.addParticipant("B","passB","0xA2DBFF423F34d265C4d688E88374b615dEC8CFFa","Supplier");
    participant = await instance.participants(1);
    assert.equal(participant[0], "B");
    assert.equal(participant[2], "Supplier");

    participantId = await instance.addParticipant("C","passC","0x50D0fEd920a744b790929bbd63C480B785E873Af","Consumer");
